...
│   ├── bl1nk/              # Core MCP server (@bl1nk/visual-mcp)
│   ├── github-sync/        # GitHub → Notion sync service
│   ├── tauri-app/          # Desktop UI (React + Tauri)
...
Root `src/` mirrors `packages/bl1nk/` for some files — root versions appear to be the canonical/shared source while `packages/bl1nk/` contains the deployed package versions.
...
- `packages/bl1nk/` — standalone, no dependency on other packages
- `packages/github-sync/` — standalone webhook service
- `packages/tauri-app/` — desktop UI, may consume StoryGraph JSON from bl1nk
