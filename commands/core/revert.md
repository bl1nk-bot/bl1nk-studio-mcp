...
Your workflow MUST anticipate and handle common non-linear Git histories, such as:

- Rewritten commits (from rebase/squash)
- Merge commits
...
### Step 1: Initiate Revert Process

Your first action is to determine the user's target.
...
**Step 2:** Immediately call the `ask_user` tool to confirm the selection (do not repeat the question in the chat):
...
- If **"yes"**: Establish this as the `target_intent` and proceed to Phase 2
- If **"no"**: Immediately call the `ask_user` tool to ask clarifying questions (do not repeat the question in the chat):
...
**Step 1: Identify Revert Candidates**

Your primary goal is to find relevant items for the user to revert.
...
**Step 2: Present a Unified Hierarchical Menu**

Immediately call the `ask_user` tool to present the results (do not list them in the chat first):
...
**Step 4: Halt on Failure**

If no completed items are found to present as options:
...
- **Handle "Ghost" Commits (Rewritten History):** If a SHA from a plan is not found in Git:
  - Announce this
  - Search the Git log for a commit with a highly similar message
  - Ask the user to confirm it as the replacement
  - If not confirmed, halt

### Step 2: Identify Associated Plan-Update Commits
...
- For each commit in the final list:
  - Check for complexities like merge commits
  - Warn about any cherry-pick duplicates
...
Present a summary of your investigation and the exact actions you will take:
...
### Step 2: Final Go/No-Go

Immediately call the `ask_user` tool to ask for final confirmation (do not repeat the question in the chat):
...
questions:
  - header: "Confirm Plan"
    ...
    options:
      - label: "Approve"
        description: "Proceed with the revert actions."
      - label: "Revise"
        description: "I want to change the revert plan."
```
...
Run `git revert --no-edit <sha>` for each commit in your final list.

**Start from the most recent and work backward.**

### Step 2: Handle Conflicts

**If any revert command fails due to a merge conflict:**

- Halt
- Provide the user with clear instructions for manual resolution

### Step 3: Verify Plan State

After all reverts succeed, read the relevant **Implementation Plan** file(s) again to ensure the reverted item has been correctly reset.

**If not:** Perform a file edit to fix it and commit the correction.

### Step 4: Announce Completion

Inform the user that the process is complete and the plan is synchronized.
