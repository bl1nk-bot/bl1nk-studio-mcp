...
export function TaskTracker({ tasks, onToggle, onAdd }: TaskTrackerProps) {
  ...
  const PriorityIcon = ({ p }: { p: Task["priority"] }) =>
    p === "high" ? <AlertCircle size={12} style={{ color: priorityColors[p] }} /> :
    p === "medium" ? <Clock size={12} style={{ color: priorityColors[p] }} /> :
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="px-6 py-5 border-b shrink-0" style={{ borderColor: "var(--border)" }}>
        <h2 className="text-base font-semibold mb-3" style={{ color: "var(--text-primary)" }}>Task Tracker</h2>
        ...
        <div className="mb-3">
          ...
          <div className="h-1 rounded-full overflow-hidden" style={{ background: "rgba(0,188,212,0.1)" }}>
            <div
              className="h-full rounded-full transition-all duration-500"
              style={{ width: `${progress}%`, background: "linear-gradient(90deg, var(--teal), var(--teal-bright))" }}
            />
          </div>
        </div>

        <div className="flex items-center gap-1 p-1 rounded-lg" style={{ background: "rgba(0,188,212,0.04)", border: "1px solid var(--border)" }}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-1">
        ...
        {visible.map(task => (
          <button
            ...
            onMouseEnter={e => !task.done && (e.currentTarget.style.borderColor = "var(--border)")}
          ...
          >
            ...
            <span className={cn("flex-1 text-sm", task.done && "line-through")} style={{ color: "var(--text-primary)" }}>
              {task.text}
            </span>
      ...
      <div className="px-4 py-3 border-t shrink-0" style={{ borderColor: "var(--border)", background: "var(--bg-elevated)" }}>
