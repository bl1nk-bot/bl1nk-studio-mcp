    ...
    <div className="flex flex-1 min-h-0 overflow-hidden" style={{ background: "var(--bg-base)" }}>
      <div className="flex flex-col flex-1 min-w-0 p-6">
        ...
        <div className="grid mb-2" style={{ gridTemplateColumns: "repeat(7, 1fr)" }}>
          {DAYS.map(d => (
            <div key={d} className="text-center text-xs font-semibold tracking-wider pb-2" style={{ color: "var(--text-muted)" }}>
              {d}
            </div>
          ))}
        </div>

        <div className="grid gap-1 flex-1" style={{ gridTemplateColumns: "repeat(7, 1fr)", gridAutoRows: "1fr" }}>
          {cells.map((day, i) => {
            ...
            return (
              ...
              >
                <span
                  ...
                  style={{
                    color: isToday ? "var(--teal)" : isSelected ? "var(--teal)" : "var(--text-secondary)",
                  }}
                >
                  {day}
                </span>
                <div className="flex gap-0.5 flex-wrap justify-center">
                  {dayNotes.slice(0, 3).map((n, ni) => {
                    const labelColor = n.labelIds?.[0] ? labelMap[n.labelIds[0]]?.color : null;
                    return (
                      <div
                        key={ni}
        ...
        <div className="flex items-center gap-3 pt-4 mt-4" style={{ borderTop: "1px solid var(--border)" }}>
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-xs" style={{ color: "var(--text-muted)" }}>Note updated on this day</span>
          </div>
          <div className="flex items-center gap-1.5">
            <div className="w-4 h-4 rounded border" style={{ border: "1px solid rgba(0,188,212,0.35)", background: "rgba(0,188,212,0.07)" }} />
            ...
            >
              ...
              <div className="min-w-0">
                ...
                {(note.labelIds ?? []).map(lid => {
                  ...
                  return (
                    <span key={lid} className="text-xs px-1 rounded" style={{ color: label.color }}>{label.name}</span>
                  );
                })}
              </div>
            </button>
