      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(210px, 1fr))" }}>
        {notes.map(note => (
          ...
          >
            ...
            <div className="text-sm font-semibold mb-1.5 line-clamp-1" style={{ color: "var(--text-primary)" }}>
              {note.title}
            </div>
            <div
            ...
            <div className="text-xs" style={{ color: "var(--text-muted)" }}>{relTime(note.updatedAt)}</div>
          </button>
        ))}
      </div>
    </div>
