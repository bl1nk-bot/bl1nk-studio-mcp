...
export function BookShelfLayout({ items, actions }: LayoutProps) {
	...
	const counts: Record<string, number> = {
		...
		Reading: items.filter(
			(i) => String(i.fields.status ?? "").toLowerCase() === "reading",
		).length,
		"To Read": items.filter(
			(i) => String(i.fields.status ?? "").toLowerCase() === "to read",
		).length,
		Completed: items.filter(
			(i) => String(i.fields.status ?? "").toLowerCase() === "completed",
		).length,
		"On Hold": items.filter(
			(i) => String(i.fields.status ?? "").toLowerCase() === "on hold",
		).length,
	};
					...
					<div className="text-xl font-bold text-blue-700">
						{
							items.filter(
								(i) =>
									String(i.fields.status ?? "").toLowerCase() === "reading",
							).length
						}
					</div>
					...
					<div className="text-xl font-bold text-emerald-700">
						{
							items.filter(
								(i) =>
									String(i.fields.status ?? "").toLowerCase() === "completed",
							).length
						}
					</div>
					...
					<div className="text-xl font-bold text-amber-700">
						{
							items.filter(
								(i) =>
									String(i.fields.status ?? "").toLowerCase() === "to read",
							).length
						}
					</div>
			...
			{filtered.length === 0 ? (
				<div className="flex flex-col items-center justify-center py-20 text-center">
					...
					<p className="mt-1 text-xs text-slate-400">
						{filter === "All"
							? "Connect your Craft space to load your books"
