...
import { CheckSquare, Square, Plus, ChevronDown } from "lucide-react";
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden" style={{ background: "var(--bg-base)" }}>
      ...
      >
        ...
        <div className="relative h-1.5 rounded-full overflow-hidden mb-4" style={{ background: "rgba(0,188,212,0.1)" }}>
          <div
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <div className="flex gap-1">
                {(["high", "medium", "low"] as Task["priority"][]).map(p => (
                  <button
                    ...
                    style={{
                      background: newPriority === p ? PRIORITY_COLORS[p] + "33" : "transparent",
              ...
              <button
                ...
                style={{
                  background: newText.trim() ? "rgba(0,188,212,0.2)" : "transparent",
        ...
        {filtered.map(task => (
          <button
            ...
            className={cn(
              ...
              !task.noteId ? "hover:bg-[rgba(0,188,212,0.05)] cursor-pointer" : "cursor-default opacity-75"
            )}
          >
            <span className="mt-0.5 shrink-0" style={{ color: task.done ? "var(--teal)" : "var(--text-muted)" }}>
              {task.done ? <CheckSquare size={15} /> : <Square size={15} />}
            </span>
            <span
              className={cn("text-sm flex-1 leading-relaxed", task.done && "line-through")}
