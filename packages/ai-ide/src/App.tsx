...
import { Label, labelsStore } from "./store/labels";
...
import {
  ...
  FileText, CheckSquare, Layout, LayoutGrid,
} from "lucide-react";
...
export default function App() {
  ...
  const handleAddTask = useCallback((text: string, priority: Task["priority"]) => {
    const task: Task = { id: `task-${Date.now()}`, text, done: false, priority };
    setStandaloneTaskList(prev => {
  ...
  const sidebarProps = {
    ...
    onNewNote: () => { handleNewNote(); setDrawerOpen(false); },
        ...
        <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
          {showEditor && (
            <Editor
              ...
              onBack={activePanel === "views" ? () => setActiveNoteId(null) : undefined}
            />
          )}
          {showTaskPanel && (
            <TaskTracker
          ...
          {showViewsPanel && (
            <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
              ...
              {view === "list"     && <ListView     notes={notes} labels={labels} onSelect={id => { selectNote(id); }} />}
              {view === "fleet"    && <FleetView    notes={notes} labels={labels} onSelect={id => { selectNote(id); }} />}
              {view === "table"    && <TableView    notes={notes} labels={labels} onSelect={id => { selectNote(id); }} onLabelToggle={handleLabelToggle} />}
              {view === "calendar" && <CalendarView notes={notes} labels={labels} onSelect={id => { selectNote(id); }} />}
              {view === "timeline" && <TimelineView notes={notes} labels={labels} onSelect={id => { selectNote(id); }} />}
        ...
        >
          {BOTTOM_NAV.map(({ icon, panel, label }) => (
            <button
              ...
              style={{
                color: activePanel === panel ? "var(--teal)" : "var(--text-muted)",
                background: activePanel === panel ? "rgba(0,188,212,0.08)" : "transparent",
              }}
            >
              {icon}
              <span className="text-[10px] font-medium capitalize tracking-wide">{label}</span>
            </button>
          ))}
        </nav>
