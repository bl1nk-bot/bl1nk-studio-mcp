...
export const AGENT_MODES: Record<AgentMode, AgentConfig> = {
	code: {
		...
		systemPrompt:
			"You are an expert coding assistant. Help users write, review, and debug code.",
	},
	ask: {
		mode: "ask",
	...
	plan: {
		...
		systemPrompt:
			"You are a project planning assistant. Help users break down tasks and create roadmaps.",
	},
	debug: {
		mode: "debug",
		skills: ["debugging", "error-analysis", "troubleshooting"],
		systemPrompt:
			"You are a debugging expert. Help users identify and fix issues in their code.",
	},
	support: {
		mode: "support",
	...
	switch (command) {
		case "agents": {
			...
			const agentList = Object.values(AGENT_MODES)
				.map(
					(a) =>
						`- ${a.mode}: ${a.systemPrompt?.split(".")[0] || "No description"}`,
				)
				.join("\n");
			return {
				newMode: currentMode,
		...
		case "switch": {
			...
			throw new Error(
				`Unknown agent mode: ${args}. Use /agents to see available modes.`,
			);
		}
		...
		default:
			throw new Error(
				`Unknown command: /${command}. Use /agents to see available commands.`,
			);
	}
}
 ...
 */
export function cycleAgentMode(
	currentMode: AgentMode,
	direction: "next" | "prev",
): AgentMode {
