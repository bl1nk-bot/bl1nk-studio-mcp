		...
		<div className="flex h-screen bg-black text-white">
			...
			<div className="w-96 border-r border-white/10 p-6 flex flex-col gap-6">
				...
				<div className="space-y-3">
					<div className="flex items-center justify-between">
						<h3 className="text-xs font-semibold text-white/30 uppercase">
							Agent Mode
						</h3>
					...
					<div
						className={`p-3 border rounded-md ${currentMode === "support" ? "border-blue-500/30 bg-blue-500/10" : "border-white/10 bg-white/5"}`}
					>
				...
				<div className="space-y-3">
					...
					<div className="p-3 border border-blue-500/30 bg-blue-500/10 rounded-md">
						...
						<p className="text-[10px] text-white/50 mt-1">
							Found in: /.claude/skills/order-lookup/
						</p>
					</div>
					<div className="p-3 border border-green-500/30 bg-green-500/10 rounded-md">
						<div className="flex items-center gap-2 text-sm font-medium">
							<ShieldCheck size={14} className="text-green-400" /> Billing
							Support
						</div>
						<p className="text-[10px] text-white/50 mt-1">
							Found in: /.claude/skills/billing/
						</p>
					</div>
				</div>
				...
				<div className="flex-1 overflow-y-auto space-y-4">
					{messages.map((m) => (
						...
						>
							<div
								className={`w-8 h-8 rounded-full flex items-center justify-center ${m.role === "user" ? "bg-blue-500" : "bg-green-500"}`}
							>
								{m.role === "user" ? <User size={16} /> : <Bot size={16} />}
							</div>
							<div
								className={`max-w-[80%] p-3 rounded-lg ${m.role === "user" ? "bg-blue-500/20" : "bg-white/10"}`}
							>
			...
			<div className="flex-1 p-6 overflow-y-auto">
				...
				<div className="mt-8 p-4 border border-white/10 rounded-lg bg-white/5">
					...
					<div className="space-y-1 text-sm text-white/60">
						...
						<p>
							Mode: <span className="text-white/80">{currentMode}</span>
						</p>
						<p>
