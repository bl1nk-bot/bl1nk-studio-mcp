...
function App(): React.ReactElement {
	...
	function handleNoteChange(id: string, content: string) {
		...
		// Sync tasks from notes while preserving standalone tasks
		const noteTasks = extractTasksFromNotes(next);
		const standaloneTasks = tasks.filter(t => !t.noteId);
		const mergedTasks = [...standaloneTasks, ...noteTasks];
		
		setTasks(mergedTasks);
		notesStore.tasksSave(mergedTasks);
	}
	...
	function handleToggleTask(id: string) {
		...
		if (task.noteId) {
			...
			// Fix: Preserve standalone tasks when syncing note-linked tasks
			const noteTasks = extractTasksFromNotes(updatedNotes);
			const standaloneTasks = tasks.filter(t => !t.noteId);
			const mergedTasks = [...standaloneTasks, ...noteTasks];
